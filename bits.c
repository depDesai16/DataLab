...
int bitXor(int x, int y) {
  ...
  int noty = ~y;
  int xandy = x & y;
  int notxandnoty = notx & noty;
  int notxandy = ~xandy;
  int notnotxandnoty = ~notxandnoty;
...
int howManyBits(int x) {
	int all = ~0x00;
  	...
  	found = (!!(x & mask)) << 4;
  	position |= (0x10 & found);
  	mask = (all << (position + 0x08));
  	found = (!!(x & mask)) << 3;
  	position |= (0x08 & found);
  	mask = (all << (position + 0x04));
  	found = (!!(x & mask)) << 2;
  	position |= (0x04 & found);
  	mask = (all << (position + 0x02));
 	found = (!!(x & mask)) << 1;
  	position |= (0x02 & found);
  	mask = (all << (position + 0x01));
  	found = !!(x & mask);
  	position |= (0x01 & found);
  	position += 1;
  	return position + (1 & x_is_zero);
}
//float
/* 
